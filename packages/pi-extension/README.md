...
| `Ctrl+Q P` | 使用与词条语言匹配的系统语音朗读当前单词 |
| `Ctrl+Q G` | 评分：good（记住了） |
| `Ctrl+Q H` | 评分：hard（有点难） |
| `Ctrl+Q A` | 评分：again（没记住） |
| `Ctrl+Q E` | 评分：easy（轻松） |

按下 `Ctrl+Q` 后，学习栏左侧会显示可用操作键；1.5 秒内没有继续输入会自动取消。评分快捷键在词卡视图和详情面板内均有效。详情面板内还额外支持 `P` 单键朗读和 `G` / `H` / `A` / `E` 单键评分。

朗读会根据词条的 BCP 47 语言标签选择本机语音，例如英语 `en`、日语 `ja-JP`、韩语 `ko-KR`。首版使用 macOS `say`；其他平台会显示暂不支持的提示，后续可以通过同一语音适配层接入。
...
| `/fw-pronounce` | 使用与词条语言匹配的系统语音朗读当前单词 |
